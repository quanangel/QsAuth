...
class Captcha {
    protected static $_config = [
        ...
        // captcha length
        'length' => 5,
        // captcha font ttf, empty is rand
        'fontttf' => '',
        // bg color
        'bg' => [243, 251, 254],
    ];
     ...
     * Name:   create captcha
     * Note:
     * Time:   2021/04/08 18:21
     * @return array
     */
    private static function create_captcha() {
        // image width
        $width = empty(self::$_config['image_w']) ? 0 : (int)self::$_config['image_w'];
        if (0 == $width) {
            $width = (int)(self::$_config['length'] * self::$_config['font_size'] * 1.5 + self::$_config['length'] * self::$_config['font_size'] / 2);
        }
        // image height
        $height = (empty(self::$_config['image_h']) || 0 == self::$_config['image_h']) ? (int)(self::$_config['font_size'] * 2.5) : (int)self::$_config['image_h'];

        // create image by width,height
        $im = imagecreate($width, $height);

        // bg color
        $color = imagecolorallocate($im, mt_rand(1, 150), mt_rand(1, 150), mt_rand(1, 150));
        // font color
        $fontColor = imagecolorallocate($im, mt_rand(151, 255), mt_rand(151, 255), mt_rand(151, 255));
        // font ttf path
        $ttfPath = __DIR__ . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . (self::$_config['use_zh'] ? 'zhttfs' : 'ttfs') . DIRECTORY_SEPARATOR;

        // handle use fontttf
        $fontttf = null;
        if (empty(self::$_config['fontttf'])) {
            $dir = dir($ttfPath);
            $ttfs = [];
        ...
        // draw background
        if (self::$_config['use_img_bg']) {
            self::background($im, $width, $height);
        }

        // draw noise
        if (self::$_config['use_noise']) {
            self::writeNoise($im, $width, $height);
        }

        // draw curve
        if (self::$_config['use_curve']) {
            self::writeCurve($im, $width, $height, (int)self::$_config['font_size'], $color);
        }
        ...
        // captcha left margin
        $codeNX = 0;
        if (self::$_config['use_zh']) {
            for ($i = 0; $i < self::$_config['length']; $i++) {
                $code[$i] = iconv_substr(self::$_config['zh_set'], floor(mt_rand(0, mb_strlen(self::$_config['zh_set'], 'utf-8') - 1)), 1, 'utf-8');
                imagettftext($im, self::$_config['font_size'], mt_rand(-40, 40), self::$_config['font_size'] * ($i + 1) * 1.5, self::$_config['font_size'] + mt_rand(10, 20), $color, $fontttf, $code[$i]);
            }
        } else {
            for ($i = 0; $i < self::$_config['length']; $i++) {
        ...
        imagedestroy($im);
        $imgBase64 = 'data:image/png;base64,' . base64_encode($content);

        return ['type' => 'image/png', 'code' => $code, 'key' => $codeKey, 'image' => $imgBase64];
    }
    ...
    private static function writeCurve(mixed $im, int $width, int $height, int $fontSize, mixed $color) {
        ...
        // 曲线前部分
        $A = mt_rand(1, (int)($height / 2)); // 振幅
        $b = mt_rand(-(int)($height / 4), (int)($height / 4)); // Y轴方向偏移量
        $f = mt_rand(-(int)($height / 4), (int)($height / 4)); // X轴方向偏移量
        $T = mt_rand($height, $width * 2); // 周期
        $w = (2 * M_PI) / $T;

        $px1 = 0; // 曲线横坐标起始位置
        $px2 = mt_rand((int)($width / 2), (int)($width * 0.8)); // 曲线横坐标结束位置
     ...
     * Name:   encrypt code
     * Note:
     * Time:   2021/04/08 18:17
     * @param  string $code
     * @param  string $microTime
     * @return string
     */
    private static function encrypt_code(string $code, string $microTime) {
        return md5($code . self::$_config['key'] . $microTime);
    }
}
